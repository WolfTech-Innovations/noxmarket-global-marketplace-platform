  ...
  function schedSave() {
    ...
    saveTimer = setTimeout(async () => {
      ...
      const payload = {
        ...
        weekday_weights:   JSON.stringify(profile.weekday_weights),
        seen:              JSON.stringify(profile.seen),
        interactions:      profile.interactions,
        pixel_cats:        JSON.stringify(profile.pixel_cats||[]),
        dwell_history:     JSON.stringify((profile.dwell_history||[]).slice(-20).map(r3)),
        price_clicks:      JSON.stringify((profile.price_clicks||[]).slice(-10).map(r3)),
        cat_history:       JSON.stringify((profile.cat_history||[]).slice(-20).map(r3)),
        like_count:        profile.like_count||0,
        watch_count:       profile.watch_count||0,
        skip_count:        profile.skip_count||0,
        share_count:       profile.share_count||0,
        buy_count:         profile.buy_count||0,
        session_count:     profile.session_count||0,
        last_visit_ts:     profile.last_visit_ts||0,
        last_updated:      profile.last_updated,
        consent_version:   profile.consent_version,
        copy_events:       profile.copy_events||0,
        tab_switch_count:  profile.tab_switch_count||0,
        idle_sessions:     profile.idle_sessions||0,
        selection_count:   profile.selection_count||0,
        long_press_count:  profile.long_press_count||0,
        page_history:      JSON.stringify((profile.page_history||[]).slice(-30)),
        entry_sources:     JSON.stringify((profile.entry_sources||[]).slice(-10)),
      };
      try {
