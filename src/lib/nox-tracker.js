  ...
  function schedSave() {
    ...
    saveTimer = setTimeout(async () => {
      ...
      const payload = {
        ...
        interest_snapshot: JSON.stringify(profile.uv.map(r3)),
        ...
        cat_cooccurrence:  JSON.stringify((profile.cat_cooccurrence||[]).slice(0,256).map(r3)),
        device_dims:       JSON.stringify(profile.device_dims||{}),
        bio_dims:          JSON.stringify(profile.bio_dims||{}),
        page_history:      JSON.stringify((profile.page_history||[]).slice(-50)),
        entry_sources:     JSON.stringify((profile.entry_sources||[]).slice(-20)),
        viewport_history:  JSON.stringify((profile.viewport_history||[]).slice(-20)),
      };
      try {
