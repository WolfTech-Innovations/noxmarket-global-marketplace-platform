...
// Detects what the current platform can actually do and gates metrics accordingly.
// Prevents wasting cycles on APIs that don't exist (iOS battery, Android WebGL2, etc.)
const PlatformCaps = (() => {
  ...
  const chromium = /chrome|chromium/.test(ua) && !safari;

  const caps = {
    // Fingerprinting
    battery:        !ios && 'getBattery' in navigator,
    ...
    audio_ctx:      'OfflineAudioContext' in window,
    font_canvas:    (() => { try { document.createElement('canvas').getContext('2d'); return true; } catch { return false; } })(),
    // Behavioral
    ...
    // APIs
    performance_mem: 'memory' in performance,
    long_tasks:     (() => { try { new PerformanceObserver(()=>{}).observe({entryTypes:['longtask']}); return true; } catch { return false; } })(),
    network_info:   !!(navigator.connection || navigator.mozConnection),
    clipboard_read: 'clipboard' in navigator && 'read' in (navigator.clipboard||{}),
    wake_lock:      'wakeLock' in navigator,
    xr:             'xr' in navigator,
    // Platform flags
    ios, android, mobile, safari, firefox, chromium,
    // Color/display
    hdr:            window.matchMedia('(dynamic-range: high)').matches,
    p3_gamut:       window.matchMedia('(color-gamut: p3)').matches,
    // Bio reliability: mouse metrics meaningless on pure touch, touch metrics meaningless on desktop
    bio_mouse_reliable: window.matchMedia('(pointer: fine)').matches,
    bio_touch_reliable: 'ontouchstart' in window,
    // LocalStorage quota estimate
    storage_ok:     (() => { try { localStorage.setItem('_nox_t','1'); localStorage.removeItem('_nox_t'); return true; } catch { return false; } })(),
  };

  // Gate: returns true if a named metric group should be collected on this platform
  function allows(metric) {
    switch(metric) {
      case 'battery':        return caps.battery;
      case 'webgl':          return caps.webgl;
      case 'webgpu':         return caps.webgpu;
      case 'audio_fp':       return caps.audio_ctx;
      case 'font_fp':        return caps.font_canvas;
      case 'mouse_bio':      return caps.bio_mouse_reliable;
      case 'touch_bio':      return caps.bio_touch_reliable;
      case 'mouse_gesture':  return caps.bio_mouse_reliable;
      case 'gamepad':        return caps.gamepad;
      case 'network_info':   return caps.network_info;
      case 'perf_mem':       return caps.performance_mem;
      case 'long_tasks':     return caps.long_tasks;
      case 'xr':             return caps.xr;
      case 'color_pixel':    return caps.webgl || caps.font_canvas; // canvas color sampling
      case 'session_store':  return caps.storage_ok;
      default: return true;
    }
  }
...
async function getWebGLFingerprint() {
  if (!PlatformCaps.allows('webgl')) return { score: 0.5, tier: 0.5, maxTexSize: 0.5, extCount: 0.5, spScore: 0.5, vendor: 0.5 };
  try {
    const c = document.createElement('canvas');
...
async function getAudioFingerprint() {
  if (!PlatformCaps.allows('audio_fp')) return { hash: 0.5, peak: 0.5 };
  try {
...
function getFontFingerprint() {
  if (!PlatformCaps.allows('font_fp')) return 0.5;
  try {
...
async function getBatteryFingerprint() {
  if (!PlatformCaps.allows('battery')) return { level: 0.5, charging: 0.5, chargingTime: 0.5, dischargingTime: 0.5 };
  try {
...
async function getWebGPUFingerprint() {
  if (!PlatformCaps.allows('webgpu')) return { tier: 0.5, score: 0.5, maxBuf: 0.5 };
  try {
...
function extendedScore(id, iv, profile, w) {
  ...
  // ── Visual palette affinity ────────────────────────────────────────────────
  const palDims = VisualPaletteTracker.getDims();
  // Sample the item's thumbnail and get its dominant hue bin
  const itemEl = document.querySelector(`[data-id="${id}"]`);
  if (itemEl) {
    const img = itemEl.querySelector('img');
    if (img && img.complete && img.naturalWidth > 4 && PlatformCaps.allows('color_pixel')) {
      try {
        const tmpC = document.createElement('canvas');
        tmpC.width = tmpC.height = 4;
        const tmpX = tmpC.getContext('2d', { willReadFrequently: true });
        tmpX.drawImage(img, 0, 0, 4, 4);
        const px = tmpX.getImageData(0,0,4,4).data;
        let rS=0,gS=0,bS=0,cnt=0;
        for (let i=0;i<px.length;i+=4){if(px[i+3]>128){rS+=px[i];gS+=px[i+1];bS+=px[i+2];cnt++;}}
        if (cnt > 0) {
          const r=rS/cnt/255, g=gS/cnt/255, b=bS/cnt/255;
          const mx=Math.max(r,g,b), mn=Math.min(r,g,b), d=mx-mn;
          let h=0; if(d>0){if(mx===r)h=((g-b)/d+(g<b?6:0))/6;else if(mx===g)h=((b-r)/d+2)/6;else h=((r-g)/d+4)/6;}
          const hueBin = Math.min(Math.floor(h*12),11);
          s += (palDims[`hue_${hueBin}`] - 0.5) * 0.025;
          const sat = d / (mx+mn<1 ? mx+mn : 2-mx-mn + 1e-9);
          s += sat > 0.5 ? (palDims.sat_high - 0.5)*0.015 : (palDims.sat_low - 0.5)*0.015;
          const bri = (mx+mn)/2;
          s += bri > 0.5 ? (palDims.bri_high - 0.5)*0.012 : (palDims.bri_low - 0.5)*0.012;
        }
      } catch {}
    }
  }
  // High contrast pref users respond to bold, high-contrast thumbnails
  s += (palDims.contrast_pref - 0.5) * 0.01;
...
// ─── VISUAL PALETTE TRACKER ───────────────────────────────────────────────────
// Samples pixel colors from thumbnail images the user dwells on, building a
// HSL-space aesthetic preference map without knowing any category labels.
// This produces an 18-dim "color taste" vector:
//   hue buckets 0-11 (30° each), sat bucket (low/high), brightness bucket (low/high), contrast score
const VisualPaletteTracker = (() => {
  const HUE_BINS   = 12;
  const huePref    = new Array(HUE_BINS).fill(0.5);  // engagement signal per hue range
  const satPref    = [0.5, 0.5];                      // [desaturated, vivid]
  const briPref    = [0.5, 0.5];                      // [dark, bright]
  let   contrastPref = 0.5;                           // prefers high-contrast content
  const paletteSamples = [];                          // recent { hue, sat, bri, signal }
  let   offCanvas  = null, offCtx = null;
  const SAMPLE_PX  = 16; // downsample to 16×16 grid for speed

  function getOffscreenCtx() {
    if (offCtx) return offCtx;
    offCanvas = document.createElement('canvas');
    offCanvas.width = offCanvas.height = SAMPLE_PX;
  ...
  function sampleImage(img, signal) {
    if (!PlatformCaps.allows('color_pixel')) return;
    ...
    try {
      ...
      const hueBuckets = new Array(HUE_BINS).fill(0);
      let totalSat = 0, totalBri = 0, pixCount = 0;
      let maxBri = 0, minBri = 1;
      for (let i = 0; i < px.length; i += 4) {
        ...
        hueBuckets[bin]++;
        totalSat += s; totalBri += l;
        if (l > maxBri) maxBri = l;
        if (l < minBri) minBri = l;
        pixCount++;
      }
      if (pixCount < 4) return;
      const avgSat = totalSat/pixCount;
      const avgBri = totalBri/pixCount;
      const contrast = cl(maxBri - minBri);
      const lr = 0.06;
      hueBuckets.forEach((cnt, i) => {
        const weight = cnt / pixCount;
        if (weight > 0.05) huePref[i] = cl(lerp(huePref[i], signal, lr * weight * 3));
      });
      satPref[0] = cl(lerp(satPref[0], signal, lr * (1-avgSat)));
      satPref[1] = cl(lerp(satPref[1], signal, lr * avgSat));
      briPref[0] = cl(lerp(briPref[0], signal, lr * (1-avgBri)));
      briPref[1] = cl(lerp(briPref[1], signal, lr * avgBri));
      contrastPref = cl(lerp(contrastPref, signal, lr * contrast));
      paletteSamples.push({ avgHue: hueBuckets.indexOf(Math.max(...hueBuckets))/HUE_BINS, avgSat, avgBri, contrast, signal });
      if (paletteSamples.length > 80) paletteSamples.shift();
    } catch {}
  }

  // Called when a slide enters view — sample all its images
  function sampleSlide(id, signal) {
    const el = document.querySelector(`[data-id="${id}"]`);
    if (!el) return;
    el.querySelectorAll('img').forEach(img => sampleImage(img, signal));
  }

  function getDims() {
    return {
      hue_0:  huePref[0],  hue_1:  huePref[1],  hue_2:  huePref[2],
      hue_3:  huePref[3],  hue_4:  huePref[4],  hue_5:  huePref[5],
      hue_6:  huePref[6],  hue_7:  huePref[7],  hue_8:  huePref[8],
      hue_9:  huePref[9],  hue_10: huePref[10], hue_11: huePref[11],
      sat_low: satPref[0], sat_high: satPref[1],
      bri_low: briPref[0], bri_high: briPref[1],
      contrast_pref: contrastPref,
      palette_var: cl(vari(huePref)*4),
    };
  }

  return { sampleSlide, sampleImage, getDims };
})();


// ─── RL POLICY GRADIENT OPTIMIZER ─────────────────────────────────────────────
// Branch architecture: each "branch" is a subset of scoring features.
// The agent maintains a softmax policy over branch weights and uses REINFORCE
// to shift weight toward branches that predicted engagement correctly.
// Reward = engagement signal. Baseline = exponential moving average of rewards.
// This replaces the simple hit/miss AdaptiveWeights system with actual RL.
const RLPolicyOptimizer = (() => {
  const BRANCHES = {
    // Branch name → array of weight keys it governs
    visual:    ['cosine', 'quality', 'novelty'],
    price:     ['price_centroid'],
    social:    ['social_proof', 'pixel_cat'],
    temporal:  ['tod', 'recency', 'reengagement'],
    bio:       ['biometric', 'tab_focus', 'idle_pattern', 'copy_signal'],
    momentum:  ['momentum', 'resize_class'],
  };
  const BRANCH_NAMES = Object.keys(BRANCHES);
  const N = BRANCH_NAMES.length;

  // Branch log-weights (θ params) — start uniform
  let theta = new Array(N).fill(0.0);
  // Per-branch advantage accumulator
  const advantages = BRANCH_NAMES.map(() => []);
  // EMA baseline for reward normalization
  let baseline = 0.5;
  const BASELINE_LR = 0.1;
  const THETA_LR    = 0.08;
  const ENTROPY_REG = 0.01; // encourage exploration
  const STORAGE_KEY = 'nox_rl_theta';

  function load() {
    try {
      const saved = JSON.parse(localStorage.getItem(STORAGE_KEY)||'null');
      if (saved && saved.length === N) theta = saved;
    } catch {}
  }

  function save() {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(theta.map(t=>Math.round(t*1000)/1000))); } catch {}
  }

  // Softmax over theta → branch multipliers
  function softmax() {
    const maxT = Math.max(...theta);
    const exps = theta.map(t => Math.exp(t - maxT));
    const sum  = exps.reduce((a,b)=>a+b,0);
    return exps.map(e => e/sum);
  }

  // Returns a multiplier map: weight_key → scale factor
  function getBranchMultipliers() {
    const probs = softmax();
    const mults = {};
    BRANCH_NAMES.forEach((name, i) => {
      BRANCHES[name].forEach(key => { mults[key] = (mults[key]||1) * (0.5 + probs[i] * N * 0.5); });
    });
    return mults;
  }

  // Called after each interaction with the actual engagement signal
  function update(signal, featureContribs) {
    const reward = signal; // 0–1, higher = more engaged
    baseline = lerp(baseline, reward, BASELINE_LR);
    const advantage = reward - baseline;

    const probs = softmax();
    BRANCH_NAMES.forEach((name, i) => {
      // Compute how much this branch contributed to the actual score
      const branchContrib = BRANCHES[name].reduce((s, key) => s + Math.abs(featureContribs[key]||0), 0);
      const normalized = cl(branchContrib / (Object.values(featureContribs).reduce((a,b)=>a+Math.abs(b),0)||1));
      // REINFORCE gradient: ∂J/∂θᵢ = advantage * (δᵢⱼ - pⱼ) * contribution
      // Simplified: gradient proportional to advantage × (indicator - prob)
      const grad = advantage * normalized * (1 - probs[i]); // policy gradient
      theta[i] = Math.max(-3, Math.min(3, theta[i] + THETA_LR * grad));
      // Entropy regularization: pull theta toward 0 (uniform)
      theta[i] *= (1 - ENTROPY_REG);
      advantages[i].push(advantage);
      if (advantages[i].length > 30) advantages[i].shift();
    });
    save();
  }
  ...
  function getDims() {
    const probs = softmax();
    const out = {};
    BRANCH_NAMES.forEach((n, i) => { out[`rl_branch_${n}`] = cl(probs[i]); });
    out.rl_baseline = cl(baseline);
    out.rl_entropy  = cl(-probs.reduce((s,p)=>s+p*Math.log2(p+1e-9),0) / Math.log2(N));
    return out;
  }

  return { load, getBranchMultipliers, update, getDims };
})();


// ─── COLD-START LOADER ────────────────────────────────────────────────────────
// Bootstraps a new user's profile from browser-available signals before any
// NoxMarket interactions. Reads:
//   • localStorage keys from other sites (same-origin only — we can only read our own)
//   • sessionStorage
//   • Referrer chain and URL params (infers category intent)
//   • navigator.language / timezone / platform (demographic proxy)
//   • Time-of-day and day-of-week at first visit (behavioral prior)
//   • Browser history length (navigator.history.length — rough session depth)
//   • Cookie presence hints (logged-in somewhere? repeat visitor patterns)
//   • Performance timing (device class inference without fingerprint)
// Returns a partial profile overlay that seeds the user vector.
const ColdStartLoader = (() => {
  function infer() {
    const seed = {};

    // ── Language/locale → geography/demographic proxy ──
    const lang = navigator.language || '';
    seed.lang_hash = hashNorm(lang);
    seed.tz_offset = cl((new Date().getTimezoneOffset() + 720) / 1440);
    seed.locale_hash = hashNorm(lang.split('-')[0]);

    // ── Time-of-day behavioral prior ──
    const h = new Date().getHours();
    seed.tod = h < 6 ? 0 : h < 12 ? 0.25 : h < 18 ? 0.5 : h < 22 ? 0.75 : 1.0;
    seed.is_weekend = ([0,6].includes(new Date().getDay())) ? 1 : 0;

    // ── Referrer intent inference ──
    const ref = document.referrer.toLowerCase();
    const url = location.href.toLowerCase();
    const searchEngines = ['google','bing','duckduckgo','yahoo','baidu'];
    seed.from_search = searchEngines.some(s=>ref.includes(s)) ? 1 : 0;
    seed.from_social  = ['reddit','twitter','facebook','discord','t.co','x.com'].some(s=>ref.includes(s)) ? 1 : 0;

    // ── URL param hints ──
    const params = new URLSearchParams(location.search);
    const utmMedium = (params.get('utm_medium')||'').toLowerCase();
    seed.from_email  = utmMedium.includes('email') ? 1 : 0;
    seed.from_paid   = ['cpc','ppc','paid'].some(s=>utmMedium.includes(s)) ? 1 : 0;

    // ── Category intent from URL/referrer ──
    const combined = url + ' ' + ref;
    const catHints = CFG.CATS.filter(c => combined.includes(c));
    if (catHints.length) {
      seed.cold_cat_hint = catHints[0];
      seed.cold_cat_idx  = catIdx(catHints[0]);
    }

    // ── Device class from performance timing ──
    try {
      const nav = performance.getEntriesByType('navigation')[0];
      if (nav) seed.cold_perf_class = cl(1 - (nav.loadEventEnd - nav.startTime) / 10000);
    } catch {}

    // ── History length (session depth proxy) ──
    seed.history_depth = cl(Math.min(history.length, 50) / 50);

    // ── Platform/device tier ──
    seed.is_mobile  = PlatformCaps.mobile ? 1 : 0;
    seed.hw_conc    = cl((navigator.hardwareConcurrency||2) / 16);
    seed.dev_mem    = cl((navigator.deviceMemory||2) / 16);
    seed.has_touch  = PlatformCaps.touch ? 1 : 0;
    seed.has_mouse  = PlatformCaps.mouse ? 1 : 0;

    // ── Cross-site localStorage scan for nox signals ──
    // We can only read our own origin's storage, but look for any nox keys
    // that might have been set by the Nox Deals extension or other NoxMarket pages
    try {
      const extVec = localStorage.getItem('nox_deals_vector');
      if (extVec) {
        const parsed = JSON.parse(extVec);
        if (Array.isArray(parsed) && parsed.length >= 8) seed.ext_vec = parsed.slice(0,32);
      }
      const extCats = localStorage.getItem('nox_deals_cats');
      if (extCats) seed.ext_cats = JSON.parse(extCats);
      const visitCount = parseInt(localStorage.getItem('nox_visit_count')||'0');
      seed.returning = visitCount > 0 ? 1 : 0;
      seed.visit_count = cl(visitCount / 50);
    } catch {}

    // ── Screen class ──
    seed.screen_class = screen.width < 400 ? 0.2 : screen.width < 768 ? 0.5 : screen.width < 1200 ? 0.75 : 1.0;
    seed.pixel_ratio  = cl((window.devicePixelRatio||1) / 4);
    seed.prefers_dark = window.matchMedia('(prefers-color-scheme: dark)').matches ? 1 : 0;

    return seed;
  }

  // Apply seed to a fresh profile's uv vector
  function applyToProfile(profile, seed) {
    if (!seed) return;
    // Temporal priors
    const todD = seed.tod < 0.25 ? 0 : seed.tod < 0.5 ? 1 : seed.tod < 0.75 ? 2 : 3;
    profile.tod_weights = profile.tod_weights.map((w,i) => i===todD ? cl(w*0.7+0.3) : w*0.9+0.025);
    if (seed.is_weekend) {
      [0,6].forEach(d => { profile.weekday_weights[d] = cl(profile.weekday_weights[d]*0.7+0.3); });
    }
    // Category hint
    if (seed.cold_cat_hint) {
      const ci = CFG.CATS.indexOf(seed.cold_cat_hint);
      if (ci >= 0) {
        profile.uv[1] = cl(profile.uv[1]*0.7 + seed.cold_cat_idx*0.3);
        profile.pixel_cats = [...new Set([...(profile.pixel_cats||[]), seed.cold_cat_hint])];
      }
    }
    // Device dims
    profile.uv[62] = seed.prefers_dark ?? 0.5;
    if (seed.hw_conc) profile.uv[45] = seed.hw_conc;
    if (seed.dev_mem) profile.uv[46] = seed.dev_mem;
    profile.uv[50] = seed.is_mobile ?? 0.5;
    profile.uv[51] = seed.screen_class ?? 0.5;
    profile.uv[52] = seed.pixel_ratio ?? 0.5;
    // Referrer type priors
    if (seed.from_search) profile.uv[6] = cl(profile.uv[6]*0.8+0.2);
    if (seed.from_social)  profile.uv[7] = cl(profile.uv[7]*0.8+0.2);
    // Ext vector seed (from Nox Deals browser extension)
    if (seed.ext_vec) {
      seed.ext_vec.forEach((v, i) => { profile.uv[i] = cl(lerp(profile.uv[i], v, 0.25)); });
    }
    if (seed.ext_cats) {
      profile.pixel_cats = [...new Set([...(profile.pixel_cats||[]), ...seed.ext_cats])];
    }
    // Mark cold-started so we don't double-apply
    profile._cold_started = true;
    return profile;
  }

  return { infer, applyToProfile };
})();


// ─── FUZZY FINGERPRINT LINKER ─────────────────────────────────────────────────
// Allows users to recover their algorithm on a new device even when localStorage
// is wiped. Collects ~12 stable hardware signals, stores a fuzzy hash of them,
// and on new devices computes the same hash and checks the server for a match.
// "Fuzzy" = we quantize each signal into coarse buckets so minor hardware diffs
// (driver updates, OS patches) don't break the link.
const FingerprintLinker = (() => {
  const LINK_KEY  = 'nox_fp_link';
  const LINK_PATH = '/api/tracker/link';

  // Quantize a float to N coarse steps — small hardware changes won't shift buckets
  function q(v, steps) { return Math.round(cl(v) * steps); }

  async function buildStableFingerprint(dd) {
    // Use only signals that are STABLE across OS reinstalls / driver updates:
    // screen res, language, timezone, platform, hardware concurrency, device memory,
    // and coarse GPU tier. Intentionally coarse so it survives minor changes.
    const parts = [
      q(dd.screen_w, 8),           // screen width bucket (8 levels)
      q(dd.screen_h, 6),           // screen height bucket
      q(dd.color_depth/32, 4),     // color depth bucket
      navigator.language?.split('-')[0] || '',
      Intl.DateTimeFormat().resolvedOptions().timeZone || '',
      navigator.platform?.slice(0,8) || '',
      q((navigator.hardwareConcurrency||2)/32, 6),
      q((navigator.deviceMemory||2)/32, 5),
      q(dd.webgl_tier||0.5, 4),    // GPU tier bucket (0-4)
      q(dd.pixel_ratio, 4),
      PlatformCaps.mobile ? 'm' : 'd', // mobile vs desktop
      PlatformCaps.touch  ? 't' : 'p', // touch vs pointer
    ];
    let h = 0x811c9dc5;
    const str = parts.join('|');
    for (let i = 0; i < str.length; i++) { h ^= str.charCodeAt(i); h = (Math.imul(h, 0x01000193)) >>> 0; }
    return 'nfp_' + h.toString(36);
  }

  // On init: build stable FP, store locally, and check server for existing profile
  async function tryLink(dd, currentUid) {
    try {
      const fpKey = await buildStableFingerprint(dd);
      const stored = localStorage.getItem(LINK_KEY);

      if (!stored) {
        // First time on this device: register this FP → UID mapping
        localStorage.setItem(LINK_KEY, fpKey);
        await fetch(LINK_PATH, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ fpKey, uid: currentUid }),
        }).catch(()=>{});
        return null; // no existing profile found
      }

      if (stored === fpKey) return null; // same device, same FP — nothing to do

      // FP changed (new device or wipe) — look up if server knows this FP
      const res = await fetch(`${LINK_PATH}?fp=${encodeURIComponent(fpKey)}`).catch(()=>null);
      if (!res || !res.ok) return null;
      const data = await res.json().catch(()=>null);
      if (data?.uid && data.uid !== currentUid) {
        // Found a linked profile! Return the linked UID so we can load that profile
        localStorage.setItem(LINK_KEY, fpKey);
        console.log(`[NoxTracker] FP link found: recovering profile from ${data.uid}`);
        return data.uid;
      }
    } catch {}
    return null;
  ...
  return { tryLink, buildStableFingerprint };
})();
...
export const NoxTracker = (() => {
  ...
  async function loadProfile(dd) {
    ...
    // ── Fuzzy fingerprint link: try to recover profile from new device ──
    const linkedUid = await FingerprintLinker.tryLink(dd, uid);
    if (linkedUid) uid = linkedUid; // use linked UID to load the recovered profile
    ...
    try {
      ...
      if (obj) {
        ...
        // Cold-start: seed the fresh profile from browser signals
        const coldSeed = ColdStartLoader.infer();
        ColdStartLoader.applyToProfile(profile, coldSeed);
        cosmicId=null;
      }
  ...
  function updateDerivedDims() {
    ...
    applyDimsAt(VisualPaletteTracker.getDims(),     650); // 18 dims → 650–667
    applyDimsAt(RLPolicyOptimizer.getDims(),        670); // 8  dims → 670–677
  }
  ...
  function score(id) {
    ...
    const baseW=AdaptiveWeights.get();
    const rlMults=RLPolicyOptimizer.getBranchMultipliers();
    const w=Object.fromEntries(Object.entries(baseW).map(([k,v])=>[k,v*(rlMults[k]??1)]));
    const contribs={};
  ...
  function onSlideEnter(id) {
    if (currentId&&watchStart) {
      ...
      RLPolicyOptimizer.update(sig, lastFeatureContribs);
      nudge(currentId, sig);
      updateBaseline(elapsed, scrollVel);
      VisualPaletteTracker.sampleSlide(currentId, sig);
  ...
  async function init() {
    ...
    // VisualPaletteTracker is passive — no attach needed, it runs on slide enter
    ...
    window.__nox={
      ...
      getTrackerDims: () => ({
        ...
        palette:    VisualPaletteTracker.getDims(),
        rl:         RLPolicyOptimizer.getDims(),
      }),
    ...
    const vidDims = VideoEngagementTracker.getDims();
    console.log(
      `[NoxTracker] v7 | uid:${uid} | dim:${CFG.VEC_DIM} | interactions:${profile.interactions}` +
      ...
      ` | trackers:23 | active_dims:${CFG.VEC_DIM}` +
      ...
      ` | rl_entropy:${RLPolicyOptimizer.getDims().rl_entropy?.toFixed(2)}` +
      ` | platform:${PlatformCaps.mobile?'mobile':'desktop'}|${PlatformCaps.ios?'ios':PlatformCaps.android?'android':'other'}`
    );
  }
